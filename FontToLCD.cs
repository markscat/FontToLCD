...
namespace FontToLCD
{
    ...
    public partial class FontToLCD : Form
    {
        ...
        public FontToLCD()
        {
            ...
            this.DotFontReview.Paint += new PaintEventHandler(this.DotFontReview_Paint);
        ...
        private void InitForm()
        {
            ...
            // 按鈕事件
            SingleLetterArray.Click += Button1_Click;
        ...
        private void DotFontGen_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    CharacterListBox.Items.Add(c.ToString());//錯誤：引數 1: 無法從 '方法群組' 轉換成 'object'
                }
            ...
            var generated = _fontGenerator.GetGeneratedCharacters();//<<名稱 '_font_generator' 不存在於目前的內容中
            MessageBox.Show($"已生成字元數: {generated.Count}");
#if _Debug
            if (generated.Count > 0)
            {
                char sample = generated.OrderBy(c => c).First();
                var r = _fontGenerator.GetResultForCharacter(sample);
                MessageBox.Show($"示例 '{sample}' -> Bitmap: {(r?.VisualBitmap != null ? "OK" : "NULL")}, C_ArrayString length: {(r?.C_ArrayString?.Length ?? 0)}");
            }
#endif
        ...
        {
#if _Debug
            //MessageBox.Show("事件觸發");

            //MessageBox.Show($"DotFontReview.Visible={DotFontReview.Visible}, Size={DotFontReview.Width}x{DotFontReview.Height}");
#endif
            ...
            GlyphSize.Text = result.GlyphBounds == Rectangle.Empty
                ? "0x0 pixels"
                : $"{result.GlyphBounds.Width}x{result.GlyphBounds.Height} pixels";
            Array_Dimensions.Text = $"[{result.ArrayDimensions.Height}][{result.ArrayDimensions.Width}]";
            Format.Text = "Horizontal Scan";

            // === 更新 C 陣列 ===
            FullFontArray.Text = string.IsNullOrEmpty(result.C_ArrayString)
                ? "// 無陣列資料"
                : result.C_ArrayString;

            // 顯示 C 陣列（若無則顯示提示）
            if (result.VisualBitmap != null)
            {
                DotFontReview.SizeMode = PictureBoxSizeMode.Zoom; // 或 PictureBoxSizeMode.CenterImage, 看你想要怎樣縮放
                DotFontReview.Image = result.VisualBitmap;
            }
            else
            {
                DotFontReview.Image = null;
                MessageBox.Show($"字元 {selectedChar} 的 Bitmap 尚未生成！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        ...
        private void DotFontReview_Paint(object sender, PaintEventArgs e)
        {
            ...
            else { e.Graphics.DrawImage(_currentPreviewBitmap, 0, 0); }
